    ...
    switch (action.type) {
        ...
        if(currentItem) {
            return {...state, cartItems: state.cartItems
                .map(x => x.product === currentItem.product ? {...item, qty: item.qty + x.qty} : x )}
        }
        else {
        ...
        case UPDATE_QTY:
            ...
            if(findItemToUpdate) {
                return {...state, cartItems: state.cartItems
                    .map(x => x.product === findItemToUpDate.product ? {...findItem,qty:findItem.qty} : x)}
            }
            else {
        ...
        case DELETE_ITEM: 
            ...
            return{...state, cartItems: state.cartItems(x => x.product !== id)}
